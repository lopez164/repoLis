 ...
 * A LBChart2D object is one with it's category descriptor labels along the bottom (or x) axis.
 * For example, a vertical bar chart is a LBChart2D type chart.
 * Changes through its set methods are updated upon next repaint() or getImage() calls.
 */
public final class LBChart2D extends GraphChart2D {


  private boolean needsUpdate;
  private LBChartArea chart;
  ...
  public final Dimension getPreferredSize() {
    ...
    if (!customizePrefSize) {
      ...
      chart.resetLBChartAreaModel (true);
    ...
    if (!chart.getBackgroundExistence()) {
      ...
      imageG2D.setRenderingHint (
        RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
      chart.updateLBChartArea (imageG2D);
    }
    else {
  ...
  public final boolean validate (boolean debug) {
    ...
    if (debug) System.out.println ("Validating LBChart2D");
    ...
    if (debug) {
      ...
      if (valid) System.out.println ("LBChart2D was valid");
      else System.out.println ("LBChart2D was invalid");
    }
  ...
  /**
   * Gets the LBChartArea of this LBChart2D.
   * @return The LBChartArea of this LBChart2D.
   */
  ...
  /**
   * Gets the chart type, LABELS_BOTTOM.
   * @return The type of the chart, LABELS_BOTTOM.
   */
  final int getGraphChartType() {
    return LABELS_BOTTOM;
  }
