...
package aplicacionlogistica.ejemplosGraficos;
...
/**
 * An y Axis for chart graphs.  Supports both vertical and horizontal bar
 * charts, low number of points line and scatter charts.  Bullets can be either
 * aligned with the labels as in vertical bar charts or between the labels as
 * int horizontal bar charts.  Label font size, type, style, and color are
 ...
 * its font may also be customized.  The title may be justified within the
 * width of the axis, bottom, top or center. Supports all of bordered areas
 * customizability except for auto min sizing.
 ...
 * recommended that the auto justification be disabled.  The yAxis must be
 * butted up against a graph area, this is best done manually.
 * This is its default.
 */
final class YAxisArea extends AxisArea {


  private VerticalTextListArea textList;
  ...
  /**
   * Creates a new YAxisArea with the following default settings:<br>
   * setAutoSizes (false, false);<br>
   * setAutoJustifys (false, false);<br>
   * setTitleJustifications (LEFT, CENTER);<br>
   * setTicksSizeModel (new Dimension (3, 3));<br>
   * setType (LABELSBOTTOM);<br>
   * setTextListJustifications (RIGHT, CENTER);<br>
   * setLabels (new String[0]);<br>
   * setTicksColor (Color.black);<br>
   * setNumTicks (0);<br>
   * setTitleRotateLeft (true);<br>
   * setBackgroundExistence (false);<br>
   * setBorderExistences (false, false, false, false);<br>
   * setGapExistences (false, false, false, false);<br>
   * textList.setBulletsRelation (TOP);<br>
   * textList.setAutoJustifys (false, false);<br>
   * textList.setBorderExistences (false, false, false, false);<br>
   * textList.setGapExistences (false, false, false, false);<br>
   * textList.setBackgroundExistence (false);<br>
   * resetYAxisModel (true);<br>
   */
  YAxisArea() {

    textList = new VerticalTextListArea();
  ...
  /**
   * Specifies the type of the y Axis.  The type refers to what type of chart
  ...
  final void setType (int type) {
    ...
    needsUpdate = true;
    if (type == LABELSBOTTOM) textList.setBulletsAlignment (CENTERED);
    else textList.setBulletsAlignment (BETWEEN);
  }


  /**
   * The color for the ticks.
   * @param color The y Axis tick color.
   */
  ...
  /**
   * The vertical alignment of the ticks respective to the labels.  The
 ...
 /**
   ...
   * @param num The number of y axis ticks.
   */
  ...
  /**
   * The existence of a gap between each label and the next.  If the gap does
   * not exist, then it will not be used in calculations.
   * @param existence The existence of a gap between each label and the next.  If
   * true, then they do.
   */
  final void setBetweenLabelsGapExistence (boolean existence) {

    needsUpdate = true;
    textList.setBetweenLabelsGapExistence (existence);
  }
   ...
   * Get for advanced configuration of the y axis.
   * @return This y axis component.
   */
  final VerticalTextListArea getTextList() {
   ...
   * and sizes of each actual tick.  The lowest order array tick is the top
   * most tick.
   * @param g2D The graphics context to use for calculations.
   ...
   */
  final boolean getYAxisAreaNeedsUpdate() {

    return (needsUpdate || getTitledAreaNeedsUpdate() ||
      textList.getVerticalTextListAreaNeedsUpdate());
  }
   ...
   * @param g2D The graphics context to use for calculations.
   */
  final void updateYAxisArea (Graphics2D g2D) {

    if (getYAxisAreaNeedsUpdate()) {
      updateTitledArea (g2D);
      update (g2D);
      textList.updateVerticalTextListArea(g2D);
    }
    needsUpdate = false;
  }
   ...
   * sizing.
   */
  final void resetYAxisModel (boolean reset) {

    needsUpdate = true;
    resetTitledAreaModel (reset);
    textList.resetVerticalTextListAreaModel (reset);
  }
  ...
  private void updateMinSizes (Graphics2D g2D) {
    ...
    if (!getAutoSize(MIN)) {
      ...
      if (titleSize.width > 0) {
        minWidth = titleSize.width + getBetweenTitleAndSpaceGapThickness (g2D) +
          textListSize.width;
      }
      else minWidth = titleSize.width + textListSize.width;
      int minHeight = titleSize.height > textListSize.height ?
        titleSize.height : textListSize.height;
  ...
  private void updateMinTextList (Graphics2D g2D) {
    ...
    int spaceHeight = textList.getSize (MIN).height;
    textList.setAllowSelfSize (false);
    spaceHeight = spaceHeight < getSpaceSize(MIN).height ?
      getSpaceSize(MIN).height : spaceHeight;
    int betweenHeight =
      textList.getSize (MIN).height - textList.getSpaceSize (MIN).height;
    spaceHeight = spaceHeight - betweenHeight;
    textList.setSpaceSize (MIN,
      new Dimension (textList.getSpaceSize(MIN).width, spaceHeight));

    int spaceY = getSpaceSizeLocation(MIN).y;
    int spaceX;
    int betweenWidth =
      textList.getSize (MIN).width - textList.getSpaceSize (MIN).width;
    if (textList.getJustifications (HORIZONTAL) == LEFT)
      spaceX = getSpaceSizeLocation (MIN).x + getTitleSize (MIN, g2D).width +
        getBetweenTitleAndSpaceGapThickness (g2D) +
        textList.getSpaceSize (MIN).width + betweenWidth / 2;
    else
      spaceX = getSpaceSizeLocation (MIN).x + getSpaceSize (MIN).width -
        textList.getSize (MIN).width + betweenWidth / 2;
